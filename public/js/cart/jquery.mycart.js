...
//購物車介面
    ...
    var drawTable = function () {
      ...
      $.each(products, function () {
        ...
        $cartTable.append(
          ...
          '<td>' + this.name + '</td>' +
          '<td title="Unit Price" class="text-right">'+'單價:<br>'+ options.currencySymbol + MathHelper.getRoundedNumber(this.price) + '</td>' +
          '<td title="Quantity"><input type="number" min="1" style="width: 70px;" class="' + classProductQuantity + '" value="' + this.quantity + '"/></td>' +
          '<td title="Total" class="text-right ' + classProductTotal + '">總價:<br>' + options.currencySymbol + MathHelper.getRoundedNumber(total) + '</td>' +
    ...
    */
    ...
    $(document).on("input", "." + classProductQuantity, function () {
      ...
      $(this).parent("td").next("." + classProductTotal).html('總價:<br>'+options.currencySymbol + MathHelper.getRoundedNumber(price * quantity));
    ...
    $(document).on('click', targetSelector, function () {
      ...
      if(account!=''){
          ...
          $.ajax({
              type: "post",
              url: url,
              data:{account:account,products:localStorage.products,_token:_token},
              // dataType: "json",
              success: function(res) {
        ...
        localStorage.products=''
          alertify.alert()
            .setting({
              'label':'確定',
              'message': '請先登入帳號' ,
              'onok': function(){ alertify.success('帳號未登錄無法進行交易');}
            }).show();
      }
    });
